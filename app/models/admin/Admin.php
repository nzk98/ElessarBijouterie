...
class Admin {
    ...
    public static function verifyCurrentPassword($id, $currentPassword) {
        $db = Database::getInstance();
        $stmt = $db->prepare("SELECT MotdePasse FROM admin WHERE ID_Admin = ?");
        $stmt->execute([$id]);
        $admin = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($admin) {
            return password_verify($currentPassword, $admin['MotdePasse']);
        }
        return false;
    }
} 